...
namespace TokenAuthentication.API.Controllers
{
    ...
    public partial class AccountController : BaseApiController
    {
        ...
        [Route("create")]
        [HttpPost]
        public async Task<IHttpActionResult> CreateUser([FromBody]UserModel createUserModel)
        {
            ...
            dbUser = new ApplicationUser()
            {
                ...
                IsAdministrator = createUserModel.IsAdministrator,
            };
            addUserResult = await this.AppUserManager.CreateAsync(dbUser, createUserModel.Password);
            ...
            //affecter role User to resistred user
            IdentityResult addResult = null;
            if (createUserModel.IsAdministrator)
            {
                addResult = await this.AppUserManager.AddToRolesAsync(dbUser.Id, "Admin");
            }
            else
            {
                addResult = await this.AppUserManager.AddToRolesAsync(dbUser.Id, "User");
            }



            if (Equals(addResult, null) || !addResult.Succeeded)
            {
                ModelState.AddModelError("", "Failed to add user as candidate or administrator");
                return BadRequest(ModelState);
            }
            ...
            #endregion
            Uri locationHeader = new Uri(Url.Link("GetUserById", new { id = dbUser.Id }));

            return Created(locationHeader, TheModelFactory.Create(dbUser));
        }
        ...
        [Route("update")]
        [HttpPost]
        public async Task<IHttpActionResult> UpdateUser([FromBody]UpdateUserModel updateUserModel) {
            ...
            if (!ModelState.IsValid)
            {
                ...
                dbUser = await this.AppUserManager.FindByNameAsync(updateUserModel.UserName);
                if (Equals(dbUser, null))
                {
                    ModelState.AddModelError("", $"User not found with login {updateUserModel.UserName}");
                    return BadRequest(ModelState);
                }
                dbUser.Email = updateUserModel.Email;
                dbUser.Birthdate = updateUserModel.Birthdate;
                dbUser.FirstName = updateUserModel.FirstName;
                dbUser.LastName = updateUserModel.LastName;

                addUserResult = await this.AppUserManager.UpdateAsync(dbUser);
            ...
            if (!addUserResult.Succeeded)
            {
              ...
              //delete all role  fromuser
                var currentRoles = await this.AppUserManager.GetRolesAsync(dbUser.Id);

                IdentityResult removeResult = await this.AppUserManager.RemoveFromRolesAsync(dbUser.Id, currentRoles.ToArray());
            ...
            //affecter role User to resistred user
            IdentityResult addResult = null;
            if (updateUserModel.IsAdministrator)
            {
                addResult = await this.AppUserManager.AddToRolesAsync(dbUser.Id, "Admin");
            }
            else
